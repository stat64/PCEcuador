...
<html>
    ...
    <body>
        ...
        $email_admin= $_POST['admin-name'];
        $admin='';
		$productoinfo=  ejecutarSQL::consultar("select * from usuario where email_usu='".$email_admin."'");
...
//---           -----------------------------------------------------------------------------------------------------
            ...
            //if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".strftime("%H%M%S").''.$_FILES['img']['size'].''.$_FILES['img']['name'])){
                    if(consultasSQL::InsertSQL("producto", "desc_pro, id_cat, precio_pro, stock_pro, Imagen, id_usu", "'$nameProd','$cateProd','$priceProd', '$stockProd','$nombrefileS','$admin'")){
                       echo '
